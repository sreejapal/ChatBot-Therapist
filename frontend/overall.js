// Fetch and display overall dashboard data
function loadOverallDashboard() {
    fetch('/summary?type=overall')
        .then(res => res.json())
        .then(data => {
    ...
    fetch('/analyze', {
        ...
        body: JSON.stringify({question: 'trend_summary', days: 180})
    })
    .then(res => res.json())
    .then(data => {
    ...
    fetch('/static/data/overall_sentiment_chart.png')
        .then(res => {
            if (res.ok) {
                document.getElementById('sentiment-chart').src = '/static/data/overall_sentiment_chart.png';
...
window.onload = function() {
    ...
    qaSubmit.onclick = function() {
        ...
        fetch('/analyze', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({question, days: 180})
        })
        .then(res => res.json())
        .then(data => {
