// Fetch and display weekly dashboard data
function loadWeeklyDashboard() {
    fetch('/summary?type=weekly')
        .then(res => res.json())
        .then(data => {
    ...
    fetch('/analyze', {
        ...
        body: JSON.stringify({question: 'trend_summary', days: 7})
    })
    .then(res => res.json())
    .then(data => {
    ...
    fetch('/static/data/weekly_sentiment_chart.png')
        .then(res => {
            if (res.ok) {
                document.getElementById('sentiment-chart').src = '/static/data/weekly_sentiment_chart.png';
